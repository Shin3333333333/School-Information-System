...
<style>
    ...
    .info-edit-btn[onclick]:not([data-admin]) {
        opacity: 0.4;
        cursor: not-allowed;
    }
    .info-edit-btn[onclick]:not([data-admin]):hover {
        background: var(--white);
        color: var(--gray-400);
...
<div class="policies-layout">
    ...
    <div class="info-panel">
        ...
        <div class="info-section mission">
            @if(auth()->user()->role->name === 'Admin')
            <button class="info-edit-btn" onclick="openInfoModal('mission')" title="Edit Mission" data-admin>
        ...
        <div class="info-section vision">
            @if(auth()->user()->role->name === 'Admin')
            <button class="info-edit-btn" onclick="openInfoModal('vision')" title="Edit Vision" data-admin>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                </svg>
        ...
        <div class="info-section values">
            @if(auth()->user()->role->name === 'Admin')
            <button class="info-edit-btn" onclick="openInfoModal('values')" title="Edit Core Values" data-admin>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                </svg>
...
<script>
    ...
    $('#infoForm').on('submit', function (e) {
        ...
        $.ajax({
            url:    '{{ route("policies.info.update") }}',
            method: 'POST',
    ...
    $('#policyForm').on('submit', function (e) {
        ...
        $.ajax({
            url:    editId ? '{{ route("policies.update") }}' : '{{ route("policies.store") }}',
            method: 'POST',
            data:   data,
            success: function (response) {
                loadingModal.hide();
